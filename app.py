    ...
    with st.spinner("Procesando datos y generando gráficos..."):
        ...
        if error:
            ...
            if not df_total.empty:
                ...
                col2.metric("% Precisión", f"{t_row['% Presicion de Eventos']:.2%}")
                col3.metric("Registrados", int(t_row['Eventos registrados por el sistema']))
                col4.metric("No Registrados", int(t_row['Eventos no registrados por el sistema']))

            st.markdown("### 📈 Análisis Visual (Estilo Informe Crystal)")
            ...
            with tab1:
                ...
                with col_left:
                    if results['img_global']:
                        ...
                        st.markdown("**Resumen de Precisión por Zona**")
                        cols_global = ['Zona', 'Fecha_reporte', 'Total eventos', 'Precicion de Eventos', '% Presicion de Eventos']
                        df_g = results['df_grafico'][cols_global].copy()
                        # Calcular Total
                        t_ev = df_g['Total eventos'].sum()
                        p_ev = df_g['Precicion de Eventos'].sum()
                        acc = p_ev / t_ev if t_ev > 0 else 0
                        total_g = pd.DataFrame([['TOTAL', '', t_ev, p_ev, acc]], columns=cols_global)
                        df_g = pd.concat([df_g, total_g], ignore_index=True)
                        st.dataframe(df_g.style.format({
                            'Total eventos': '{:.0f}',
                            'Precicion de Eventos': '{:.0f}',
                            '% Presicion de Eventos': '{:.2%}'
                ...
                with col_right:
                    if results['img_totales']:
                        ...
                        st.dataframe(results['df_total'][['Total eventos', '% Eventos registrados por el sistema', '% Presicion de Eventos']].style.format({
                            'Total eventos': '{:.0f}',
            ...
            with tab2:
                ...
                for i, cam in enumerate(cameras):
                    ...
                    cols_cam = ['Zona', 'Camara_pura', 'Total eventos', 'Precicion de Eventos', '% Presicion de Eventos', 'Eventos registrados por el sistema', '% Cobertura de Identity']
                    
                    # Calcular Fila Total de Cámara
                    t_c = cam_df['Total eventos'].sum()
                    p_c = cam_df['Precicion de Eventos'].sum()
                    r_c = cam_df['Eventos registrados por el sistema'].sum()
                    i_c = cam_df['Cobertura de Identity'].sum() if 'Cobertura de Identity' in cam_df.columns else 0
                    
                    acc_c = p_c / t_c if t_c > 0 else 0
                    cob_i = i_c / t_c if t_c > 0 else 0
                    
                    total_cam = pd.DataFrame([['TOTAL CÁMARA', cam, t_c, p_c, acc_c, r_c, cob_i]], columns=cols_cam)
            ...
            with tab4:
                ...
                for i, img_data in enumerate(results['zone_images']):
                    ...
                    count_cols = [
                        'Total eventos', 'Eventos registrados por el sistema', 'Eventos no registrados por el sistema', 
                        'Precicion de Eventos', 'Identity unknown', 'Cobertura Genero', 'Cobertura de Edad', 'Cobertura de Identity'
                    ]
            ...
            with tab5:
                ...
                count_cols_all = [
                    'Total eventos', 'Eventos registrados por el sistema', 'Eventos no registrados por el sistema', 
                    'Precicion de Eventos', 'Precision de Genero', 'Precicion de Edad', 'Identity unknown',
                    'Cobertura Genero', 'Cobertura de Edad', 'Cobertura de Identity'
                ]
